# usuarios/scripts/crear_soportes.py
import sys
import os
import django
import pandas as pd
...
from usuarios.models import Usuario, PerfilSoporte
...
# -----------------------------
grupo_soporte, _ = Group.objects.get_or_create(name="grupo_soportes")
...
for index, row in df.iterrows():
    try:
        ...
        # Crear o actualizar perfil de soporte
        if hasattr(user, "perfil_soporte"):
            perfil = user.perfil_soporte
            perfil.nombres = nombres
            perfil.paterno = paterno
            perfil.materno = materno
            perfil.celular = str(int(row['celular'])) if pd.notna(row['celular']) else ""
            perfil.codigo = codigo
            perfil.save()
        else:
            perfil = PerfilSoporte(
                codigo=codigo,
                usuario=user,
                nombres=nombres,
                paterno=paterno,
                materno=materno,
                celular=str(int(row['celular'])) if pd.notna(row['celular']) else ""
            )
            perfil.save()

        # Agregar usuario al grupo
        user.groups.add(grupo_soporte)
...
print("Soportes creados y actualizados con éxito ✅")
